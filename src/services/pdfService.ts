...
export const pdfService = {
  generatePDF: async (formData: any) => {
    try {
      ...
      skillQuestions.forEach(question => {
        const response = formData[question.key] || 'Não respondido';
        addMultilineField(pdf, question.label + ':', response, currentY);
        currentY += Math.max(20, Math.ceil(response.length / 80) * 6);
      ...
      conductItems.forEach(item => {
        const value = formData[item.key] || 'Não avaliado';
        addField(pdf, item.label + ':', value, currentY);
        currentY += 8;
      });
      ...
      if (formData.satisfacaoGeral) {
        ...
        climateFields.forEach(field => {
          if (formData[field.key]) {
            addField(pdf, field.label + ':', formData[field.key], currentY);
            currentY += 8;
          }
        });
      }
...
function saveEvaluationData(formData: any) {
  try {
